...
local function round(value)
  ...
  -- Skydance WZ5 legacy EF00 contract from Z2M v26.99.0
  -- (skydance.ts and legacy.ts tuya_light_wz5). All five operating modes use
  -- DP1 state, DP2 white/color mode, DP3 white brightness, DP4 color
  -- temperature and DP5 a 12-hex-digit H/S/B string. The exposed surface is
  -- mode-specific, and RGBW/RGB+CCT keep white brightness separate from the
  -- color-channel switchLevel value. These legacy definitions do not use
  -- tuyaBase, so they also have no magic/query/refresh/connection-status
  -- lifecycle and do not turn the device on before a mode+value command.
  ...
  local function wz5_white_brightness_from_device(value)
    -- Z2M maps raw 0..1000 to 0..255 but declares the white_brightness expose
    -- as 0..254. Keep the declared contract at the SmartThings schema edge.
  ...
  local function wz5_color_payload(device, hue, saturation, brightness)
    -- Z2M fillInHSB defaults to 360/1000/1000 and only replaces hue or
    -- saturation when the supplied value is truthy. Preserve that zero-value
    -- behavior while translating SmartThings' 0..100 hue to degrees.
    local degrees = 360
...
register_device_definition(avatto_zdms16_1, device_helpers.create_fingerprints("TS0601", {
  ...
  -- SmartThings represents the same wire range as the standard 0..100% level.
