...
-- Frozen Z2M valueConverter.thermostatScheduleDayMultiDPWithDayNumber:
-- one day byte followed by six [hour, minute, uint16 temperature*10] tuples.
...
local mazda = {
  ...
  tuya.dp_numeric(3, {
    name = "running_state",
    read_only = true,
    -- Frozen Z2M reports `heat`; SmartThings' official operating-state enum
    -- names the same state `heating`.
  ...
  -- DP105 frost_temperature is deliberately absent: frozen Z2M has no expose.
}
