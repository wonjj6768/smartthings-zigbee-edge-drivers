...
local NASWV03B_UNIT_EMITTERS = {
  gallons = {
    current = emit.nasThreeCurrentGallons(),
    total = emit.nasThreeTotalGallons(),
    quantitative = emit.nasThreeQuantitativeGallons(),
    surplus = emit.nasThreeSurplusGallons(),
    single_amount = emit.nasThreeSingleAmountGallons(),
  },
  liters = {
    current = emit.nasThreeCurrentLiters(),
    total = emit.nasThreeTotalLiters(),
    quantitative = emit.nasThreeQuantitativeLiters(),
    surplus = emit.nasThreeSurplusLiters(),
    single_amount = emit.nasThreeSingleAmountLiters(),
  },
}

local function naswv03b_definition(profile, unit_kind, include_on_countdown)
  local unit_emitters = assert(NASWV03B_UNIT_EMITTERS[unit_kind])
  local definition = {
    profile = profile,
    time_start = "2000",
    ...
    tuya.dp_numeric(9, { name = "naswv03b_water_current", read_only = true, emit = unit_emitters.current, converter = converter.divide_by_pair(1000) }),
    tuya.dp_battery(11, { read_only = true, emit = emit.battery() }),
    tuya.dp_numeric(15, { name = "naswv03b_water_total", read_only = true, emit = unit_emitters.total, converter = converter.divide_by_pair(1000) }),
    ...
    tuya.dp_numeric(102, { name = "naswv03b_quantitative_watering", emit = unit_emitters.quantitative }),
    ...
    tuya.dp_numeric(105, { name = "naswv03b_surplus_flow", read_only = true, emit = unit_emitters.surplus }),
    ...
    tuya.dp_numeric(108, { name = "naswv03b_single_watering_amount", read_only = true, emit = unit_emitters.single_amount }),
  }
...
-- Z2M: RTX ZVG1 (rtx.ts:11)
-- legacy fz.ZVG1: DP1 state, DP5 water consumed in fluid ounces, DP7 battery,
-- DP10 weather delay, DP11 timer seconds, DP12 timer state, DP15 last open
-- duration in seconds. DP16/DP17 raw timer programs are not exposed.
local zvg1_valve = {
...
-- Z2M: Nova Digital ZVL-PRO (tuya.ts:26258)
-- DP1 state, DP4 read-only fault, DP5 last watering volume, DP7 battery,
-- DP11 countdown seconds, DP12 read-only work state
local irrigation_zvl_pro = {
  ...
  -- Z2M parses DP4 but declares no fault expose for this exact, so it stays
  -- observable in logs without a profile capability.
...
-- Z2M: GIEX GX03 2-zone watering timer (tuya.ts:21914)
-- DP1/DP2 valve state, DP13/DP14 timer minutes, DP25/DP26 last open duration
-- in seconds, DP59 battery, DP104/DP105 read-only zone state
...
-- Z2M: Tuya TS0601_water_switch dual water valve (tuya.ts:15235)
-- Same DP numbers as GX03 but countdown is writable and the zone status enum is
-- manual/auto/idle, so it stays a separate runtime family.
...
-- Z2M: Tuya TS0601_water_valve ultrasonic water meter valve (tuya.ts:2242)
-- DP1 water consumed, DP2/DP3 raw month/day totals, DP4 report period,
-- DP5 fault bitmap, DP13 valve, DP14 auto clean, DP16 meter id,
-- DP18 reverse total, DP21 flow rate, DP22 temperature, DP26 battery voltage
