-- Wave19 Candeo/Heiman bridge source-only candidates.
-- Frozen Zigbee2MQTT v26.99.0:
--   src/devices/candeo.ts:55-141,1257-1272
--   src/devices/heiman.ts:2039-2095,2188-2243,2973-3003
--   src/lib/heiman.ts:52-65,168-252
...
local function handle_kinetic_press(device, _, context)
  ...
  -- Match frozen utils.hasAlreadyProcessedMessage(): suppress any sequence
  -- still present in the endpoint's five-transaction reorder window.
  ...
  -- Frozen Z2M supports a 500..1500 ms option and defaults to 1000 ms.
  -- Select its complete five-action policy so every frozen exposed action is
  -- reachable without inventing a SmartThings device DP for a software option.
...
local function send_heiman_command(device, command_id, payload, context)
  -- Frozen tzLocal deliberately overrides the custom cluster's manufacturer
  -- option with null. Outgoing F0/F1/F3/F4/F6 frames therefore carry no
  -- manufacturer-specific header even though inbound responses use 0x120B.
...
local function build_heiman_definition(options)
  ...
  mappings[#mappings + 1] = response_mapping(0xF201, 0xF2, parse_study_response, function(device, value)
    ...
    -- Preserve frozen converter behavior literally: result 1 is success.
