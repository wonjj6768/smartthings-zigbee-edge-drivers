...
-- The WOOX definitions use legacy EF00 reports in addition to On/Off. Their
-- datapoint datatype is not documented by Z2M, so only the verified core
-- switch contract is exposed until an exact payload log is available.
...
register_device_definition(tuya_metered_plug_core, {
  -- Raw table preserves the two trailing spaces reported by this exact fingerprint.
...
local function load_retail_plug_registrations()
  local retail_device_helpers = require "contracts.helpers.family"
  local retail_zcl_device_helpers = require "contracts.helpers.zcl"

  local registrations, register = retail_device_helpers.definition_registry()
  local retail_metered_plug = {
    profile = "plugs-switch-power-energy-voltage",
    zcl_clusters = retail_zcl_device_helpers.metering_clusters({
      include_switch = true,
      include_current = false,
    }),
  }

  register(retail_metered_plug, {
    retail_device_helpers.create_fingerprint("SONOFF", "S60ZBTPG"),
  })

  register(retail_metered_plug, {
    retail_device_helpers.create_fingerprint("Innr", "SP 242"),
    retail_device_helpers.create_fingerprint("Innr", "SP 244"),
  })

  return registrations
end

for _, registration in ipairs(load_retail_plug_registrations()) do
  device_definitions[#device_definitions + 1] = registration
end
