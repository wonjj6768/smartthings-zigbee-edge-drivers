-- HOBEIAN/ZG-222Z identity: project issue #15. Contract: ZHC e2aa714,
-- TS0207_water_leak_detector white-label ZG-222Z, standard IAS + PowerCfg.
...
local function passive_ias(mapping)
  -- ZHC receives commandStatusChangeNotification; it does not configure IAS
  -- ZoneStatus reporting or read it during setup. Retain SDK IAS enrollment.
...
local function battery_percentage(value, _, context)
  ...
  -- The common mapping has already divided the wire percentage by two.
  return context and value or raw / 2
end

local zg222z = {
  profile = "hobeian-zg222z-water-leak",
  -- Issue #15 reports this endpoint profile. Allow its ZCL RX decoding while
  -- actual APS traffic and acceptance of standard HA TX remain unverified.
  ...
  zcl_clusters = {
    ...
    -- fz.battery also accepts a reported BatteryAlarmState. Do not request or
    -- configure it: this definition configures only BatteryPercentageRemaining.
  ...
  parent_refresh = function(device, preset)
    -- The sleeping device has no ZHC writable/get converter. Refresh the same
    -- standard battery attribute read by its explicit configure contract.
