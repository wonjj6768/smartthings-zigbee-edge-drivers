...
-- WindowCovering has a protocol-level default command sender. Vendor and
-- standard writable attributes on that same cluster must bypass it and issue
-- an attribute write, otherwise values such as `reversed` are mistaken for a
-- cover motion command.
...
local function single_definition(model, profile, extras)
  ...
  -- These families use modernExtend.identify(): frozen default 3 seconds.
...
local function dynamic_definition(model, profile, extras)
  ...
  for channel = 1, 2 do
    ...
    -- These dynamic families use the local Sber converter: frozen default 30 seconds.
...
local function socket_definition()
  ...
  local commands = {}
  append(mappings, zcl.switch({ endpoint = 1, component = "main", configure_reporting = false, read_on_configure = false }))
  -- This family uses modernExtend.identify(): frozen default 3 seconds.
  add_identify(mappings, commands, mapping_prefix, capability_prefix, 1, "main", 3)
  add_power_on_behavior(mappings, mapping_prefix, capability_prefix, 1, "main")
