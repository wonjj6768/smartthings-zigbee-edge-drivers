...
- `ef00/motion.lua`
  Legacy/frozen combined motion/presence package source.
  Keep this for `ef00-motion-wonjj6768` preservation only.
  Do not edit this file or any file under `ef00/motion/`.
  `source/frozen_packages.json` locks these files and `deploy/ef00-motion-wonjj6768`.
- `ef00/motion_pir.lua`
...
4. EF00 motion/presence is the current exception: legacy `ef00-motion` is preserved, and new split packages use wrapper modules plus `include_device_files`.
5. Do not rebuild, edit, or redeploy `ef00-motion-wonjj6768`; use split packages for all new work.
...
- `source/tools/build_dist.py`
  Reads the manifest, builds deploy outputs, validates package/module references, and enforces raw size limits.
