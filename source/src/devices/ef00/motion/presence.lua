-- 모션/존재감 디바이스 정의 (PIR, 레이더, mmWave)
-- Z2M zigbee-herdsman-converters 기반 DP 그룹핑
--
-- 변수명 규칙: {category}_{variant}
--   category: pir      (PIR 모션센서, occupancy → motionSensor)
--             presence (mmWave/레이더, presence → presenceSensor)
...
local presence_model_zy_hps01 = {
  ...
  -- Z2M ZY_HPS01 (tuya.ts:21344) reports DP101 as occupancy, and every range
...
-- 2-11g. presence_switch_model_zg_302zm: ZG-302ZM (존재감 + 3구 스위치)

-- Z2M: _TZE200_kccdzaeo 등 (ZG-302ZM Motion sensing switch)

-- ══════════════════════════════════════════════════════════════

-- Z2M HOBEIAN ZG-302ZM (tuya.ts:20889)
-- DP1 presence trueFalse1, DP2 sensitivity 0..19, DP4 distance /100,
-- DP101..103 switches, DP108 trigger switch, DP111 backlight,
-- DP112 power outage memory, DP113 auto on, DP114 trigger hold, DP115 auto off
...
-- 2-11g. presence_switch_model_zg_302zl: ZG-302ZL (존재감 + 3구 스위치)

-- Z2M: _TZE200_khzbklyh 등 (ZG-302ZL Motion sensing switch)

-- ══════════════════════════════════════════════════════════════

-- Z2M HOBEIAN ZG-302ZL (tuya.ts:20992)
-- DP1..3 switches, DP14 power outage memory, DP16 backlight,
-- DP101 presence trueFalse1, DP102 sensitivity 0..19, DP103 trigger hold,
-- DP104 auto on, DP105 auto off.  No distance or trigger-switch DP here.
...
-- 2-11h. presence_switch_model_zis03: ZIS-03 / ZIS-04 (존재감 + 릴레이)

-- Z2M: _TZE204_izy1g1mb / _TZE204_f2rflfa6

-- ══════════════════════════════════════════════════════════════

-- Z2M Novato ZIS-03 / ZIS-04 (tuya.ts:28160)
-- DP1 presence (raw value 1), DP4 detection range level 1..7,
-- DP102 indicator, DP103 illuminance, DP104 fading time 2..3600 s,
-- DP106 illuminance compensation 1..10, DP107 presence relay output,
-- DP108 radar enable, DP111 detection area, DP112 state reversal,
-- DP113 sensitivity.  DP101 live distance exists only on ZIS-04.
